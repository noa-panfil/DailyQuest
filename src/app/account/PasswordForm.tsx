...
export default function PasswordForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (res?.error) {
            ...
            setMessage({ text: res.message || "Tada !", type: "success" });
            setCurrentPassword("");
            setNewPassword("");
        }
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-700 p-6 rounded-3xl shadow-xl flex flex-col gap-4">
            {message.text && (
            ...
            <div className="flex flex-col gap-1.5">
                <label className="text-sm font-bold text-slate-400 uppercase tracking-widest px-1">Mot de passe actuel</label>
                <input
                    type="password"
                    value={currentPassword}
                    onChange={(e) => setCurrentPassword(e.target.value)}
                    className="w-full bg-slate-950/50 border border-slate-700 p-3 rounded-xl text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-medium"
            ...
            <div className="flex flex-col gap-1.5">
                <label className="text-sm font-bold text-slate-400 uppercase tracking-widest px-1">Nouveau mot de passe</label>
                <input
                    type="password"
                    value={newPassword}
